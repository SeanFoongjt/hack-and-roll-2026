...
This project uses Supabase Postgres via the REST API. Schema changes are done in
Supabase (SQL editor or CLI), and server access is handled in `server/db.ts`.

### Schema Definition (Supabase SQL)

Create the `users` table in Supabase with camelCase column names to match the app:
...
Add database queries in `server/db.ts` using Supabase REST:

```ts
const url = `${SUPABASE_URL}/rest/v1/users?openId=eq.${encodeURIComponent(openId)}&limit=1`;
...
For write operations, use `Prefer: resolution=merge-duplicates` with
`?on_conflict=openId` to upsert.
...
| Issue | Solution |
|-------|----------|
| "Database not available" | Check `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` |
